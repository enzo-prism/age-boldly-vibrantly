...
- **Build chain:** Vite 5 + SWC. Production builds run three commands in sequence: `npm run sitemap` → `vite build` → `npm run prerender`. The sitemap reader pulls from `src/data/blogPosts.ts`, while the prerender script injects canonical SEO tags for `/blog` and each article into `dist/`.
- **Analytics:** Google Analytics (gtag), Hotjar, and the GPT Engineer bootstrap script are injected via `index.html`.

## Routing & Features
- **Layout:** Sticky animated header, framer-motion page transitions, and a footer with medical disclaimer plus social/email CTAs.
- **Key pages:**
  - `Home`: Hero carousel, “Three Pillars” cards, welcome modal linking to the Welcome Letter, and a global contact CTA.
...
- **Reusable CTAs:** `ConnectCTA`, `WelcomePopup`, `VideoCard`, `MobileMenuSection`, `GallerySection`, etc.

## Data & Integrations
- **Static content:** Blog metadata (`src/data/blogPosts.ts`), pillar copy, video series, and Suz’s curated content are TypeScript objects for determinism and sitemap reuse.
...
- **Typeform:** Contact + newsletter capture use embedded forms; Typeform’s loader script is cached globally when quizzes are shown.
...
- `scripts/prerender.tsx` rewrites `dist/blog/**/index.html` files post-build to include Helmet-equivalent metadata for crawlers that do not execute JS.

## Recently Added Blog Post
- **Slug:** `losing-weight-without-losing-your-sparkle`
- **Location:** Metadata entry at `src/data/blogPosts.ts` and full article block inside `src/pages/BlogPost.tsx` (search for the slug).
- **Date:** January 25, 2026. Remember to rerun `npm run sitemap && npm run build && npm run prerender` before deploying so the new slug is discoverable and prerendered.
